...
英国型では、人が目で見る画面はレンダリングの瞬間にソフトウェアが生成します。ということは、レンダラにバグがあれば——あるいは細工されていれば——**人は原本と違う金額を見て、それに合意する**。しかも保存義務の対象はデータであって画面ではないので、後から「私にはこう見えていた」を再現できません。電子署名の世界で **WYSIWYS（What You See Is What You Sign）** と呼ばれてきた古典的な難問が、そのまま出てきます。
...
そして「どちらが原本か」を決めているのは、実は 2 節で見たあのキーです。

- `AFRelationship = Alternative` — この XML は内容の**代替表現**である。すなわち人が読む面と機械が読む面は等価
- `AFRelationship = Data` — 視覚表現を**導出するための情報**である。すなわち PDF のほうが主

ISO 32000-2 が「処理系に何の処理命令も与えない」と注記したキーが、**どちらを原本として合意したのかという契約上の問いに答えている**。4 段の積み上げは、法的定義で止まらず、合意の中身まで届いていました。
...
- **乖離は英国型でも消えない。場所が移るだけ**——ファイルの中（監査できる）から、レンダリングの瞬間（痕跡が残らない）へ。そして「どちらを原本として合意したか」を決めているのは `AFRelationship` の値
