...
veraPDF が見ているのは PDF/A、つまり**入れ物**の側なので、これは veraPDF の落ち度ではありません。問題はそこではなく、**この食い違いを見る層が見当たらない**ことのほうです。私が確認した範囲では、Factur-X のバリデータも EN 16931 の Schematron も、PDF のページと XML を突き合わせることは検査対象にしていません（4 節で層ごとに分けます）。

先に断っておくと、**この記事が実際に測ったのは PDF/A の層だけ**です。他の層については「何を対象にしている仕様か」からの推論で、実行はしていません。
...
flowchart TB
    ...
    subgraph FR["フランス — 2026/09/01 認可プラットフォーム"]
        ...
        F0 --> F2
        F0 --> F3
        F3 --> F4["人向けの表現と機械向けの表現を<br/>1 ファイルに併存させる"]
    end
```
...
flowchart TB
    ...
    L4["フランス 2026/09/01〜<br/>この形式が法令上<br/>受け入れられる請求書になる"]
    L1 --> L2 --> L3 --> L4
```

**「処理系に何の指示も与えない」と仕様が明記しているキーの上に 4 段積んで、その先端で、税務上有効な請求書かどうかが決まっている。** これが今の e-invoicing の構造です。
...
:::message alert
**仕様上、①〜④のどれも、PDF のページに印字された金額と、埋め込み XML の値が一致するかを検査対象にしていません。**
...
veraPDF のバグでも、Schematron の手抜きでもない。**この検査は、標準として決められた 4 層のどこにも割り当てられていない**という話です。

やろうと思えばできる検査ではあります。実際、商用のバリデータには「PDF に見えている合計と XML の合計の一致」を検査項目として掲げているものがあります。ただしそれは**標準の外**にあり、どの事業者が、何を、どこまで見ているかは、その事業者ごとに違う。**規格が保証してくれる範囲には入っていません。**

## 5. 検証の緑は、誰が、何を、どの範囲で見て付けた緑なのか
...
| 観点                   | 英国（純データ型）                                     | 仏（Factur-X / ハイブリッド型）                                          |
| ---------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------ |
| **人が承認するとき**   | レンダラのバグや細工で、原本と違う数値を見て承認しうる | ページと埋め込み XML が食い違ったまま承認しうる                          |
| **乖離はどこに残るか** | **残らない**。画面は保存対象ではなく、再現できない     | **ファイルの中に残る**。2 つの面が同居したまま保存される                 |
| **後から検証できるか** | できない（比べる相手がいない）                         | できる。ただし**自動で比べる層は標準にない**。人が開いて見比べれば分かる |
| **原本（SSOT）**       | 1 つ（データ）。人が見た画面は法的に無関係             | 2 つ。どちらが主かは `AFRelationship` が表明する                         |

「どちらが堅牢か」ではなく、**「乖離が証拠として残るかどうか」**の違いです。ハイブリッドは乖離を抱え込むかわりに、**監査可能な状態で**抱え込む。純データは乖離を抱え込まない代わりに、乖離が起きても**痕跡が残らない**。

:::message
PDF/A は「開けること」を目指す規格であって、**改変を防ぐ規格ではありません**。今回の `07` も、`01` に別の XML を付け替えただけで作れています。固定するのは署名の仕事ですが、それも「食い違ったまま凍結する」だけです（この節の最後で触れます）。
:::

エンジニアとしては「SSOT を 1 つに絞ってくれ」と言いたくなるところです。ただ英国型は乖離を**解決した**のではなく、**片方を法的に無視することで見えなくした**とも読めます。人が画面で何を見たかは、もう問われない。SSOT は 1 つになりましたが、そのかわりに「人間の合意」という概念が制度から外れています。
...
- フランスは 2026 年 9 月 1 日から。共通基盤の 3 形式は UBL 2.1 / CII / Factur-X で、このうち PDF/A-3 と XML のハイブリッドは Factur-X（＝ドイツの ZUGFeRD 2.x と技術的に同一）
- Factur-X は、ISO 32000-2 が「処理系に指示を与えない注釈」と明記した `AFRelationship` の上に 4 段積み上げた構造の先端にある。意味を足していったのは ISO ではなく、その上の層
- **PDF/A-3b の検証（veraPDF 146 ルール）は、器を見る層である。**Factur-X としての妥当性は②の層、XML の業務ルールは③④の層の担当。ただし**仕様上、①〜④のどれも、PDF のページと XML の値の一致を検査対象にしていない**
- 「宣言」「適合」「検証」は別物。宣言は自分で名乗るだけ、適合は証明できず反証できるだけ、検証はそのルールの範囲でしか有効でない。義務化が進むと、この区別を怠ることのコストが技術的なものから法的なものに変わる
...
「PDF 請求書」という言葉は、**海峡のどちら側に立っているかを言わないともう何の意味も持たない**——というのが元記事の締めでした。作ってみた側から一つ足すなら、**緑もまた、誰が何をどの範囲で見た緑なのかを言わないと、何の意味も持たない**、です。

今回出てきた緑を全部並べると、こうなります。

| 緑を付けたもの                              | その緑が意味すること                                                                                             | 意味しないこと                                                                                                               |
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **XMP の `pdfaid` / `fx:`**<br>（宣言）     | **緑ではない。**文書が自分でそう名乗っただけ                                                                     | 何も。適合も検証も経ていない。名乗りが嘘でも書ける                                                                           |
| **veraPDF**（PDF/A-3b）<br>◎ 実測           | veraPDF 1.30.0 の **146 ルールで反証できなかった**。フォント埋め込み・ICC / OutputIntent・XMP の形式・暗号化なし | ISO 19005 への適合（誰にも証明できない）／Factur-X としての妥当性／中身の正しさ。**別ビルドなら 146 という範囲自体が変わる** |
| **Factur-X バリデータ**<br>△ 未実行         | XMP の `fx:` 宣言・ファイル名・`AFRelationship`・プロファイル宣言が仕様どおり                                    | 宣言したプロファイルの中身が実際に揃っているか（③④の担当）／**ページと XML が一致するか**                                    |
| **XSD**（CII D22B）<br>△ 未実行             | XML が構文として正しい。要素の有無・型・出現回数                                                                 | 数字が業務ルールとして整合しているか（④の担当）／意味が正しいか                                                              |
| **EN 16931 Schematron**<br>△ 未実行         | XML **内部**の業務ルールを満たす。合計＝明細の和、税率と税額の対応（BR-CO-15 等）                                | **その金額が正しいこと。**10 倍でも内部が閉じていれば通る（検体 `07` がそれ）／ページとの一致                                |
| **`validate_clauses`**（自作）<br>◎ 実測    | 同梱の制約テーブルで反証できなかった                                                                             | ISO 32000 全体への適合。`needs_external_fact` は pass ではない                                                               |
| **PAdES 署名**<br>— 今回は扱わず            | 署名後に改変されていない。その鍵で署名された                                                                     | 署名者が名乗るとおりの人か（trust anchor 次第）／**署名した中身が正しいか。食い違ったまま凍結される**                        |
| **（存在しない）**<br>PDF 面と XML 面の一致 | —                                                                                                                | —                                                                                                                            |

「意味しないこと」の列が、ほとんど**隣の層の担当**を指しているのが分かると思います。緑は一つ一つ正しく、しかも互いを補うように設計されている。**にもかかわらず、全部足しても届かない場所が一つ残る。**

### 同じ空欄は、たぶんあなたの現場にもある

これは PDF や請求書に限った話ではありません。

- **TypeScript の `tsc` が緑**でも、ビジネスロジックが正しいとは限らない
- **JSON Schema が緑**でも、業務要件を満たすとは限らない
- **OpenAPI Validator が緑**でも、API の意味は保証されない
- **電子署名が有効**でも、文書の内容が正しいとは限らない
- **AI の評価ベンチマークが高く**ても、実運用で安全とは限らない

どれも「形式が閉じているか」を見ています。意味は、形式に写像しきれなかった側に残る。そして厄介なことに、**どの層も自分が何を見ていないかを言いません。**`tsc` は「ビジネスロジックは見ていません」と断ってから緑を返したりしない。

面白いのは、制度のほうがその限界を認めていることです。EU の VAT 指令 233 条が最初に挙げるのは署名でも EDI でもなく「**監査証跡を生む業務統制**」でした。日本の電帳法施行規則第 4 条第 1 項の 4 択も、最後は「訂正削除防止の事務処理規程を定めて運用すること」です。**形式に写像しきれなかった部分を、制度は人と組織に投げ返している。**

検証の緑は、見た範囲の緑でしかありません。**その範囲を数え上げられないうちは、緑を信じる根拠もない。**
